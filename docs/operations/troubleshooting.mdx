...
diagnostic. Correct the reported boundary or runtime problem, then run:

```bash
yeet service readmit <svc>
```

Catch revalidates the isolation boundary and active runtime before clearing
quarantine. `yeet stop <svc>` does not clear quarantine.
