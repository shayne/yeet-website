...
- Existing native binaries, scripts, and scheduled jobs can now move from
  `legacy` mode into a Bubblewrap sandbox without redeploying their payload or
  environment, including services whose older units use historical runtime
  paths.
- ISO-networked native services now use the stable Catch runner for their
  network gate, so later Catch upgrades cannot strand the helper on a pruned
...
- Compatible Ubuntu hosts can now activate native Bubblewrap sandboxes while
  AppArmor restricts unprivileged user namespaces. Catch installs and verifies
  a narrow Yeet-owned profile without disabling AppArmor, changing global
  user-namespace settings, or replacing conflicting operator content.
- Sandboxed native services now complete managed-identity activation with
  explicit service roots, and troubleshooting distinguishes host-policy
  failures from older private service-root paths that need a traversable custom
  root or a separately planned storage migration.
