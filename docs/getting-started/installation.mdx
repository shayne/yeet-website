...
- You can reach the host from your laptop (LAN or tailnet). You do **not** need
  Tailscale installed locally; yeet embeds tsnet for RPC.
...
- **catch host (Tailscale/tsnet)**: the hostname that `yeet` targets for RPC
  ...
  advertised by catch (default `catch` or `--tsnet-host`).
...
builds locally and uploads the binary; when run from a released yeet (or with
`--from-github`) it downloads the latest stable release from GitHub before
installing.
