...
machine; yeet uses an embedded tsnet client for RPC.
...
If you use a non-root user, yeet will invoke sudo on the remote. `--from-github`
installs the latest release instead of building locally; add `--nightly` for
nightly.
