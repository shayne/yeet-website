...
Each identity change updates ownership and the systemd unit as one
rollback-safe operation. If your service uses a ZFS dataset, yeet preserves
that root and applies the same identity migration there instead of copying it
to the default filesystem path.
