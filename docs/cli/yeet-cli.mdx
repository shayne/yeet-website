...
- `--run-as=<user>[:<group>]`: run a native binary or script as an existing
  host account. New native services default to `yeet-svc`.
...
New cron jobs run as `yeet-svc`; use `--run-as=<user>[:<group>]` to select a
different existing host account. Use [Cron Jobs](/docs/payloads/cron-jobs) for
timer behavior.
...
service data directory and, for a native service, uses that service's persisted
UID and GID. VM services still connect to the guest operating system with SSH,
because the guest is a real machine and gets its own authentication boundary.
