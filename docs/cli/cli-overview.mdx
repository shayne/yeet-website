...
which arguments belong to yeet versus the payload. When a command behaves
unexpectedly, check those boundaries first.
...
every parser in the chain has to guess where its job ends.
...
`NO_COLOR`, redirected output, `--progress=plain`, `--progress=quiet`, and
structured JSON formats remain unstyled for scripts and logs.
